...
void Uart_Test(void)
{
    ...
    int uartFd = 0;
    char *commond = NULL;
    commond = "This is a test message!"; /* 对应手势里面字符串 */
    unsigned char uartReadBuff[2048] = {0}; /* 读取buff大小为2048 */
    unsigned int RecvLen = strlen(commond);
    unsigned int ret = 0;
    int readLen = 0;
    ...
    while (1) {
        ret = UartSend(uartFd, (unsigned char*)commond, strlen(commond));
        readLen = UartRead(uartFd, uartReadBuff, RecvLen, 3000); /* 1000 :time out */
        if (readLen > 0) {
